...
get_satellite_pass() {
    ...
    local url="${base_url}/${sat_id}/${lat}/${lon}/${alt}/7/1/?apiKey=${api_key}"
    ...
    if echo "$response" | jq -e '.error' > /dev/null 2>&1; then
        ...
        echo "API Error for ${sat_name}: $error_message" >&2
        return
    fi
    ...
    # Extract the first pass from the response
    pass=$(echo "$response" | jq -c '.passes[0]')
...
# Fetch and display passes for each NOAA satellite
