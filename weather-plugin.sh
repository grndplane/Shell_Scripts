...
function setIcons {
    ...
    # Corrected for Miles per hour MPH https://www.weather.gov/mfl/beaufort
    WIND=""
    ...
    if [ $BEAUFORTICON == "yes" ];then 
        ...
        if [ $WINDFORCE2 -le 1 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 1 ] && [ $WINDFORCE2 -le 3 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 4 ] && [ $WINDFORCE2 -le 7 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 8 ] && [ $WINDFORCE2 -le 12 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 13 ] && [ $WINDFORCE2 -le 18 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 19 ] && [ $WINDFORCE2 -le 24 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 25 ] && [ $WINDFORCE2 -le 31 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 32 ] && [ $WINDFORCE2 -le 38 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 39 ] && [ $WINDFORCE2 -le 46 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 47 ] && [ $WINDFORCE2 -le 54 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 55 ] && [ $WINDFORCE2 -le 63 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 64 ] && [ $WINDFORCE2 -le 72 ]; then
            WINDICON=""
        elif [ $WINDFORCE2 -gt 72 ]; then
            WINDICON=""
        fi
    fi
    if [ $KNOTS = "yes" ]; then
...
function outputCompact {
    OUTPUT="$WIND %{T$WEATHER_FONT_CODE}%{F$ICON_COLOR}$ICON%{F-}%{T-} $ERR_MSG$COLOR_TEXT_BEGIN$DESCRIPTION$COLOR_TEXT_END| $TEMP $HUMIDITY%{F$COLOR_COLD}%{T$TEMP_FONT_CODE}%{T-}%{F-}"
